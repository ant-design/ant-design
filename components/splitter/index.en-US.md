...
| collapsible | `motion` to enable collapse animation, `icon` to customize collapse icons. Set `aria-label` on a custom icon to override the localized control name | `{ motion?: boolean; icon?: { start?: ReactNode; end?: ReactNode } }` | - | 6.4.0, `aria-label`: 6.7.0 | × |
