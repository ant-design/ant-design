...
| onChange | Called when the page number or `pageSize` is changed. When `pageSize` changes, the third parameter `info.recommendPage` provides a recommended page number based on the new page size to help preserve the user's data position | function(page, pageSize, info?) | - | info: 6.7.0 | × |
