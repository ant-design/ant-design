...
| image | Custom Image element for an icon avatar | ReactNode | - |  |
...
> Tip: You can set `image`, `icon` or `children` as the fallback for image load error, with the priority of `image` > `icon` > `children`
