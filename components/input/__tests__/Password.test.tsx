...
describe('Input.Password', () => {
  ...
  it('restores the selection if marking a hydrated password dirty moves the caret', async () => {
    jest.useFakeTimers();
    const input = <Input.Password defaultValue="initial-password" />;
    const container = document.createElement('div');
    container.innerHTML = renderToString(input);
    const element = container.querySelector('input')!;
    element.setSelectionRange(2, 5, 'backward');
    ...
    await waitFakeTimer();
    expect(element).toHaveValue('initial-password');
    expect(element).not.toHaveAttribute('value');
    expect([element.selectionStart, element.selectionEnd, element.selectionDirection]).toEqual([
      2,
      5,
