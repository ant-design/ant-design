...
| value | 日期 | [moment](http://momentjs.com/)\[] | 无 |
| onChange | 时间发生变化的回调，只在结束时间被选定后触发 | function(dates: [moment, moment], dateStrings: [string, string]) | 无 |
| onCalendarChange | 时间发生变化的回调，开始日期或结束日期改变时都会触发 | function(dates: [moment, moment], dateStrings: [string, string]) | 无 |
| onOk | 点击确定按钮的回调 | function() | - |
