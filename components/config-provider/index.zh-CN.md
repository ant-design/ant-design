...
| upload | 设置 Upload 组件的通用属性 | { className?: string, style?: React.CSSProperties, classNames?:[UploadConfig\["classNames"\]](/components/upload-cn#semantic-dom), styles?: [UploadConfig\["styles"\]](/components/upload-cn#semantic-dom), customRequest?: [Upload\["customRequest"\]](/components/upload#api), progress?: [ProgressProps](/components/progress#api), accept?: [Upload\["accept"\]](/components/upload#api), showUploadList?: [Upload\["showUploadList"\]](/components/upload#api) } | - | 5.7.0, `customRequest`: 5.27.0, `classNames` 和 `styles`: 6.0.0, `accept`, `progress` and `showUploadList`: 6.4.0 |
