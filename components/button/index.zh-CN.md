...
| classNames | 用于自定义组件内部各语义化结构的 class。支持 `对象` 或 `函数`：对象写法用于静态 class，自定义语义结构 class；函数写法可以根据 `props` 动态生成 class。 | [Record<SemanticDOM, string> \| (info: { props })=> Record<SemanticDOM, string>](#semantic-dom) | - | 5.4.0 |
...
| styles | 用于自定义组件内部各语义化结构的行内 style。支持 `对象` 或 `函数`；对象写法用于静态 style，自定义语义结构样式；函数写法可以根据 `props` 动态生成样式。 | [Record<SemanticDOM, CSSProperties> \| (info: { props })=> Record<SemanticDOM, CSSProperties>](#semantic-dom) | - | 5.4.0 |
