...
| classNames | Customize class for each semantic structure inside the component. Supports `object` or `function`. object form is for static classes. function form allows dynamic class generation based on `props`. | [Record<SemanticDOM, string> \| (info: { props })=> Record<SemanticDOM, string>](#semantic-dom) | - | 5.4.0 |
...
| styles | Customize inline style for each semantic structure inside the component. Supports `object` or `function`. object form is for static styles. function form allows dynamic style generation based on `props`. | [Record<SemanticDOM, CSSProperties> \| (info: { props })=> Record<SemanticDOM, CSSProperties>](#semantic-dom) | - | 5.4.0 |
