...
| showUploadList | 是否展示文件列表, 可设为一个对象，用于单独设定 `showPreviewIcon` 和 `showRemoveIcon` 和 `showDownloadIcon` | Boolean or { showPreviewIcon?: boolean, showRemoveIcon?: boolean, showDownloadIcon?: boolean } | true |  |
...
| onDownload   | 点击下载文件时的方法，传方法择执行方法逻辑，不传则默认跳转新标签页。               | Function(file): void | 跳转新标签页   |  |
