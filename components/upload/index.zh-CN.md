...
| folder | 支持上传文件夹（[caniuse](https://caniuse.com/#feat=input-file-directory)）([和 directory 的区别](#faq-folder-vs-directory)) | boolean | false | 5.27.0 |
...
### folder 与 directory 的区别 {#faq-folder-vs-directory}

directory 会把 accept 之外的文件过滤掉，会在以后废弃，不建议使用。而 folder 则不会，与原生行为一致。
