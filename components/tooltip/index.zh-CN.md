...
这是由于 `rc-tooltip` 的实现方式导致的，`rc-trigger` 强求 children 能够接受 ref，不然就会 fallback 到 findDOMNode，所以 children 要么需要是原生 html 标签，要么需要使用 `React.forwardRef` 将 ref 透传到原生标签。
