...
const SkeletonImage = (props: SkeletonImageProps) => {
  const renderSkeletonImage = ({ getPrefixCls }: ConfigConsumerProps) => {
    ...
    const backgroundSizeCls = classNames({
      [`${prefixCls}-image-lg`]: size === 'large',
      [`${prefixCls}-image-sm`]: size === 'small',
    });

    const imageSize = classNames({
      [`${prefixCls}-image-svg-sm`]: size === 'small',
      [`${prefixCls}-image-svg-lg`]: size === 'large',
    });

    return (
      <div className={cls}>
        <div className={classNames(`${prefixCls}-image`, className, backgroundSizeCls)}>
          <svg
            viewBox="0 0 1098 1024"
            xmlns="http://www.w3.org/2000/svg"
            className={classNames(`${prefixCls}-image-svg`, imageSize)}
            style={{ ...style }}
          >
