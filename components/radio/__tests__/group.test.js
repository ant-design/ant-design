...
describe('Radio Group', () => {
  ...
  describe('value is null or undefined', () => {
    it('use `defaultValue` when `value` is undefined', () => {
      ...
      expect(wrapper.find('.ant-radio-checked > input').prop('value')).toEqual('bamboo');
    });
    ...
    [undefined, null].forEach(newValue => {
      it(`should set value back when value change back to ${newValue}`, () => {
        ...
        console.log(wrapper.find('.ant-radio-checked').html());
        expect(wrapper.find('.ant-radio-checked').first().length).toBe(0);
      });
    });
  });
});
