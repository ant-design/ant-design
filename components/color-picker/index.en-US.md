...
| onChange | Called when `value` is changed | `(value: Color, hex: string) => void` | - |
| onFormatChange | Called when `format` is changed | `(format: 'hex' \| 'rgb' \| 'hsb') => void` | - |
| onOpenChange | Called when `open` is changed | `(open: boolean) => void` | - |
