...
<code src="./demo/label-display.tsx">标签展示模式</code>
...
| labelDisplay | 水平 `default` 和 `dot` 步骤条的标签展示模式。`auto` 保持默认行为，`wrap` 允许标签换行，`scroll` 允许横向滚动。对于不支持的类型，或 `responsive` 触发竖排时，会回退到 `auto`。 | `auto` \| `wrap` \| `scroll` | `auto` |  |
