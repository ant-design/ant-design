...
<code src="./demo/label-display.tsx">Label Display</code>
...
| labelDisplay | Label display mode in horizontal `default` and `dot` steps. `auto` keeps default behavior, `wrap` allows multi-line labels, and `scroll` enables horizontal scrolling. For unsupported modes, and when `responsive` switches to vertical, it falls back to `auto`. | `auto` \| `wrap` \| `scroll` | `auto` |  |
