...
const BehaviorMap: React.FC<BehaviorMapProps> = ({ data }) => {
  ...
  useEffect(() => {
    G6.registerNode('behavior-start-node', {
      draw: (cfg, group) => {
        const [textWidth] = G6.Util.getTextSize(cfg?.label, 16);
        const [width, height] = [textWidth + 20 * 2, 48];
    ...
    G6.registerNode(
      ...
      {
        draw: (cfg, group) => {
          const [textWidth] = G6.Util.getTextSize(cfg?.label, 14);
          const padding = 16;
          const [width, height] = [
          ...
          if (cfg?.children) {
            ...
            group?.addShape('text', {
              ...
              attrs: {
                text: `${length}`,
                x: width + 6 - G6.Util.getTextSize(`${length}`, 12)[0] / 2,
                textBaseline: 'middle',
                fill: '#fff',
                fontSize: 12,
                cursor: 'pointer',
    ...
    graph.on('node:click', (e) => {
      const { link } = e.item?.getModel() as any;
      if (link) {
