...
// node scripts/visual-regression-report-upload.js ./visualRegressionReport.tar.gz --ref=pr-id
// node scripts/visual-regression-report-upload.js ./imageSnapshots.tar.gz --ref=master-commitId

const args = process.argv.slice(2);
if (args.length < 2) {
  console.error(
    'Usage: node scripts/visual-regression-report-upload.js <tarFilePath> --ref=<refValue>',
  );
  process.exit(1);
}
...
async function uploadFile(client, filePath, refValue) {
  ...
  try {
    ...
    console.log('Uploading file successfully: %s', r1);
  } catch (err) {
