...
- `Avatar` 的 `size` 默认值从 `default` 变更为 `medium`。

**6.3.2 引入：**

- `Badge`、`Card`、`Progress`、`Steps`、`Switch`、`Spin` 的 `size` 使用 `medium` 替代 `default`。
...
### Modal 与 Drawer 蒙层模糊效果调整（6.3.0）{#mask-blur-default}

v6.0.0 默认开启了 Modal、Drawer 的蒙层模糊效果（blur），但在 6.3.0 中**改为默认关闭**。如需使用模糊效果，可通过 ConfigProvider 手动开启：
