...
1. `fork` `antd` 并 `clone` 到本地，切换到 `feature` 分支，执行一次拉取确保最新，基于 `feature` 切换一个新分支，以下工作将在新分支完成
...
6. 在 `components/locale-provider/__tests__/index.test.js` 添加该语言的测试用例
7. 运行`npm run test -- -u` 来更新 snapshot，在这之前或许你还需要先删除 `node_modules` 和 `yarn.lock` or `package-lock.json` 并全新安装
8. 更新 `docs/react/i18n.zh-CN.md` and `docs/react/i18n.zh-CN.md`, 将对应语音添加到文档列表
9. 万事俱备等待 review
