...
6. 在 [components/locale-provider/**tests**/index.test.js](/components/locale-provider/__tests__/index.test.js) 添加该语言的测试用例。
7. 运行`npm run test -- -u` 来更新 snapshot，在这之前或许你还需要先删除 `node_modules` 和 lock 文件 （`yarn.lock` or `package-lock.json`） 并全新安装。
