...
## `undefined` 和 `null` 在 `antd` 的受控组件中有区别吗？ {#faq-undefined-null}
...
## 官方文档中没有提供的隐藏 API 我可以使用吗？ {#faq-internal-api}

不推荐。对内接口不保证兼容性，它很可能在某个版本中因重构而移除。如果你确实需要使用，需自行确保版本升级时隐藏接口仍旧可用，或者锁定版本。

## 为何新增 API 请求需要严格讨论？ {#faq-api-discussion}

我们在添加 API 时十分谨慎，因为一些 API 可能不够抽象从而变成历史债务。例如当需要对交互方式进行更改，这些不良抽象可能会引发 Breaking Change。为了避免诸如此类问题，我们推荐新功能优先通过 HOC 实现。

## 当我点击 `Select Dropdown DatePicker TimePicker Popover Popconfirm` 内的另一个 popup 组件时它会消失，如何解决？ {#faq-popup-disappear}
...
## `Select Dropdown DatePicker TimePicker Popover Popconfirm` 会跟随滚动条上下移动？ {#faq-popup-scroll}
...
## 如何修改 Ant Design 的默认主题？ {#faq-modify-theme}

可以参考[定制主题](/docs/react/customize-theme-cn)。

## 如何修改 Ant Design 组件的默认样式？ {#faq-modify-style}

你可以覆盖它们的样式，但是我们不推荐这么做。antd 是一系列 React 组件，但同样是一套设计规范。

## 如何避免升级导致的破坏性变更？ {#faq-breaking-change}
...
## 如何使用其他时间日期库如 Moment.js？ {#faq-custom-date-library}

可以参考[使用自定义日期库](/docs/react/use-custom-date-library-cn)。

## 当我动态改变 `defaultValue` 的时候它并没有生效。 {#faq-defaultValue}
...
## 多个组件放一排时没有垂直对齐怎么办？ {#faq-vertical-align}

尝试使用 [Space](/components/space-cn) 组件来使他们对齐。

## antd 覆盖了我的全局样式！ {#faq-global-style}
...
## 我没法安装 `antd` 和 `antd` 的依赖，顺便提一句，我在中国大陆。 {#faq-install-china}

那啥，试试 [npmmirror 国内镜像](https://npmmirror.com) 和 [cnpm](https://github.com/cnpm/cnpm)。

## 我在 package.json 里将 `dependencies.antd` 添加到了 git repository 中，但是没有用。 {#faq-git-repo}

当然没用了，请使用 npm 安装 `antd`。

## `message` 和 `notification` 是小写的，但是其他的组件都是首字母大写的，这是手滑吗？ {#faq-lowercase-component}

不，因为 `message` 是一个函数，而不是一个 React 组件。

## `antd` 在移动端体验不佳。 {#faq-mobile}
...
## `antd` 可以像 `React` 那样使用单文件引入吗？ {#faq-standalone}
...
## 在我的网络环境下没法获取到 `icon` 文件。 {#faq-icon-cdn}
...
## 如何拓展 antd 的组件？ {#faq-extend-component}
...
## 如何获取未导出的属性定义？ {#faq-unexported-types}
...
## 我的组件默认语言是英文的？如何切回中文的。 {#faq-default-locale}
...
## 开启了 Content Security Policy (CSP) 如何处理动态样式？ {#faq-csp-style}

你可以通过 [ConfigProvider](/components/config-provider-cn#content-security-policy) 来配置 `nonce` 属性。

## 当我指定了 DatePicker/RangePicker 的 `mode` 属性后，点击后无法选择年份/月份？ {#faq-date-picker-mode}
...
## ConfigProvider 设置 `prefixCls` 后，message/notification/Modal.confirm 生成的节点样式丢失了？ {#faq-prefix-cls-style}
...
## 为什么我不应该通过 ref 访问组件内部的 props 和 state？ {#faq-ref-internal-state}
...
## 弹层类组件为什么要统一至 `open` 属性？ {#faq-open-prop}
...
## V5 中 colorPrimary 和 colorInfo 及 colorLink 之间是什么关系？ {#faq-token-color}

在 Ant Design Token 系统中 `colorPrimary` 和 `colorInfo` 同属于 [基础变量（Seed Token）](/docs/react/customize-theme-cn#基础变量seed-token)，所以两者是互相独立的。`colorLink` 则属于 [别名变量（Alias Token）](/docs/react/customize-theme-cn#别名变量alias-token), 默认继承 `colorInfo` 且和 `colorPrimary` 无关。

## 如何正确的拼写 Ant Design？ {#faq-spelling}
...
## 你们有接受捐助的渠道吗，比如支付宝或者微信支付？ {#faq-donation}

[https://opencollective.com/ant-design](https://opencollective.com/ant-design)

## 使用表单组件的 `setFieldsValue` 方法如果对象类型中含有 `null` 时 TS 类型报错 {#faq-setFieldsValue-null}
...
## 为什么 antd 不处理浏览器缩放遇到的精度问题？ {#faq-browser-zoom}
...
## 使用 Next.js 的 App Router 时 antd 组件报错 {#faq-nextjs-app-router}
