...
| --- | --- | --- |
| Name | Two-character name: display the first character, followed by a `*`. | L\* |
|  | Names with three characters or more: display the first character and the last character, replace the middle character(s) with `*`. | A\*i <br /> A\*\*e |
...
| Address | Keep the provinces, cities and district information, followed by several `*`. | Zhejiang Province, Hangzhou, Xihu District \*\*\*\*\*\*\*\* |
