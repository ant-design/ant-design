...
Ant Design 视觉回归测试的主要目标是检测组件的视觉变化，避免 PR 改动引入的视觉问题。我们使用 Vitest 与 Puppeteer 对每一个组件 Demo 进行截图，然后与基准截图进行比较。
...
我们自建了视觉回归测试的方案，利用前面提到的 Puppeteer，将每一个组件的 Demo 分别使用 `dark`、`light`、`compact` 以及 `cssVar` 四种主题进行截图，然后将截图上传到 [阿里云 OSS](https://www.aliyun.com/product/oss) 中，作为基准截图。
